...
class Evaluete(DatasetPaths):
    ...
    def _load_rec_result(self, model: str):
        ...
        for k in papers.keys():
            ...
            if self.experiment_data == "aan" and paper["year"] != 2012:
                continue

            for r in self.ks:
                getattr(self, "recall_list_" + r)[k] = count(r)

    def _get_average(self, list: list) -> float:
    ...
    def _recall(self):
        ...
        for r in self.ks:
            recall = self._get_average([v["right"].__len__() / v["references"].__len__()
    ...
    def _ndcg(self):
        ...
        for r in self.ks:
            ndcg = self._get_average([get_one_ndcg(v["references"], v["real_recommendation"], v["right"])
                                      for k, v in getattr(self, "recall_list_" + r).items()])
            print("ndcg" + r, ndcg)
